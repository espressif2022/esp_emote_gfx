/*
 * SPDX-FileCopyrightText: 2026 Espressif Systems (Shanghai) CO LTD
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#pragma once

#include <stdbool.h>
#include <stdint.h>

#include "esp_err.h"
#include "core/gfx_obj.h"
#include "core/gfx_timer.h"

#ifdef __cplusplus
extern "C" {
#endif

/**
 * Minimal rig runtime prototype.
 *
 * Goal: let scene/widgets focus on "objects + state changes" only.
 * - The rig owns a timer and calls `tick_cb` periodically.
 * - When `tick_cb` reports changes (or `force_apply`), rig calls `apply_cb`.
 *
 * This is intentionally small for prototyping; it can be extended to node/param tables later.
 */

typedef struct gfx_rig_cfg_t {
    uint16_t timer_period_ms;
    int16_t damping_div;
} gfx_rig_cfg_t;

typedef struct gfx_rig_t gfx_rig_t;

typedef bool (*gfx_rig_tick_cb_t)(gfx_rig_t *rig, void *user_data);
typedef esp_err_t (*gfx_rig_apply_cb_t)(gfx_rig_t *rig, void *user_data, bool force_apply);

struct gfx_rig_t {
    gfx_timer_handle_t timer;
    gfx_rig_cfg_t cfg;
    gfx_disp_t *disp;
    gfx_obj_t *root;
    gfx_rig_tick_cb_t tick_cb;
    gfx_rig_apply_cb_t apply_cb;
    void *user_data;
};

void gfx_rig_cfg_init(gfx_rig_cfg_t *cfg, uint16_t timer_period_ms, int16_t damping_div);

esp_err_t gfx_rig_init(gfx_rig_t *rig,
                       gfx_disp_t *disp,
                       gfx_obj_t *root,
                       const gfx_rig_cfg_t *cfg,
                       gfx_rig_tick_cb_t tick_cb,
                       gfx_rig_apply_cb_t apply_cb,
                       void *user_data);

void gfx_rig_deinit(gfx_rig_t *rig);

esp_err_t gfx_rig_set_period(gfx_rig_t *rig, uint16_t period_ms);

/** Run one tick immediately (no wait). */
esp_err_t gfx_rig_step(gfx_rig_t *rig, bool force_apply);

/** Utility: damped step for int16 values (same policy as existing widgets). */
int16_t gfx_rig_ease_i16(int16_t cur, int16_t tgt, int16_t div);

#ifdef __cplusplus
}
#endif

