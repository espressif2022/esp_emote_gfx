    ...
    if (!player->visible) {
        for (uint8_t i = 0; i < player->seg_obj_count; i++) {
            if (player->seg_objs[i] != NULL) {
                gfx_object_set_visible(player->seg_objs[i], false);
            }
        }
        for (uint8_t i = 0; i < player->icon_obj_count; i++) {
            if (player->icon_objs[i] != NULL) {
                gfx_object_set_visible(player->icon_objs[i], false);
            }
        }
                                        ...
                                        const gfx_motion_asset_t *asset)
{
    ...
    solid_src.type = GFX_IMAGE_SRC_TYPE_IMAGE_DSC;
    solid_src.data = &player->solid_img;

    player->canvas_x = 0;
    player->canvas_y = 0;
    player->canvas_w = (uint16_t)gfx_display_get_h_res(disp);
    player->canvas_h = (uint16_t)gfx_display_get_v_res(disp);
    ...
    if (asset->icon_count > 0U && max_icon_segments > 0U) {
        gfx_image_src_t solid_icon_src = solid_src;
...
static void gfx_motion_player_deinit(gfx_motion_player_t *player)
{
    ...
    for (uint8_t i = 0; i < player->seg_obj_count; i++) {
        if (player->seg_objs[i] != NULL) {
            gfx_object_delete(player->seg_objs[i]);
            player->seg_objs[i] = NULL;
        }
    }
    for (uint8_t i = 0; i < player->icon_obj_count; i++) {
        if (player->icon_objs[i] != NULL) {
            gfx_object_delete(player->icon_objs[i]);
