...
static void gfx_anim_render_4bit_pixels(gfx_color_t *dest_buf, gfx_coord_t dest_stride,
                                        ...
                                        const gfx_eaf_header_t *header, uint32_t *palette_cache,
...
void gfx_anim_free_frame_info(gfx_anim_frame_info_t *frame)
{
    ...
    if (frame->header.width > 0) {  // Check if header data is valid instead of header_valid flag
        gfx_eaf_free_header(&frame->header);
        memset(&frame->header, 0, sizeof(gfx_eaf_header_t));  // Clear header data
    }
...
bool gfx_anim_preprocess_frame(gfx_anim_property_t *anim)
{
    ...
    const void *frame_data = gfx_eaf_format_get_frame_data(anim->file_desc, anim->current_frame);
    size_t frame_size = gfx_eaf_format_get_frame_size(anim->file_desc, anim->current_frame);
    ...
    gfx_eaf_format_t format = gfx_eaf_header_parse(frame_data, frame_size, &anim->frame.header);
    if (format != GFX_EAF_FORMAT_VALID) {
        return false;
    }

    // Pre-allocate parsing resources and calculate offsets
    const gfx_eaf_header_t *header = &anim->frame.header;
    ...
    gfx_eaf_calculate_offsets(header, anim->frame.block_offsets);
...
void gfx_draw_animation(gfx_obj_t *obj, int x1, int y1, int x2, int y2, const void *dest_buf, bool swap_color)
{
    ...
    // Process each block that overlaps with the destination area
    for (int block = 0; block < num_blocks; block++) {
        ...
        if (block != *last_block) {
            esp_err_t decode_result = gfx_eaf_block_decode(header, frame_data, block, decode_buffer, swap_color);
    ...
    for (int y = 0; y < clip_h; y++) {
        for (int x = 0; x < clip_w; x += 2) {
            ...
            if (palette_cache[index1] == 0xFFFFFFFF) {
                gfx_color_t color = gfx_eaf_palette_get_color(header, index1, swap_color);
            ...
            if (palette_cache[index2] == 0xFFFFFFFF) {
                gfx_color_t color = gfx_eaf_palette_get_color(header, index2, swap_color);
    ...
    for (int32_t y = 0; y < h; y++) {
        for (int32_t x = 0; x < w; x++) {
            ...
            if (palette_cache[index] == 0xFFFFFFFF) {
                gfx_color_t color = gfx_eaf_palette_get_color(header, index, swap_color);
