        ...
        uint8_t *out_data, size_t *out_size)
{
    ...
    GFX_GOTO_ON_ERROR(jpeg_dec_open(&config, &jpeg_dec), err, TAG, "JPEG SW decoder open failed");

    jpeg_io = malloc(sizeof(*jpeg_io));
    GFX_GOTO_ON_FALSE(jpeg_io != NULL, GFX_ERR_NO_MEM, err, TAG, "No mem for jpeg io");

    out_info = malloc(sizeof(*out_info));
    GFX_GOTO_ON_FALSE(out_info != NULL, GFX_ERR_NO_MEM, err, TAG, "No mem for jpeg header");
    ...
    GFX_GOTO_ON_FALSE(jpeg_ret == JPEG_ERR_OK, GFX_FAIL, err, TAG, "JPEG SW header parse failed");

    w = out_info->width;
    h = out_info->height;

    size_t required_size = (size_t)w * (size_t)h * 2U;
    GFX_GOTO_ON_FALSE(*out_size >= required_size, GFX_ERR_INVALID_SIZE, err, TAG,
    ...
    GFX_GOTO_ON_FALSE(jpeg_ret == JPEG_ERR_OK, GFX_FAIL, err, TAG, "JPEG SW decode failed: %d", jpeg_ret);
                                     ...
                                     uint32_t *out_width, uint32_t *out_height)
{
    ...
    GFX_GOTO_ON_ERROR(jpeg_dec_open(&config, &jpeg_dec), err, TAG, "JPEG info decoder open failed");
    ...
    jpeg_error_t jpeg_ret = jpeg_dec_parse_header(jpeg_dec, jpeg_io, out_info);
    GFX_GOTO_ON_FALSE(jpeg_ret == JPEG_ERR_OK, GFX_FAIL, err, TAG, "JPEG info header parse failed");
        ...
        uint8_t *out_data, size_t *out_size)
{
    ...
    GFX_GOTO_ON_ERROR(jpeg_dec_open(&config, &jpeg_dec), err, TAG, "JPEG RGB888 decoder open failed");
    ...
    jpeg_error_t jpeg_ret = jpeg_dec_parse_header(jpeg_dec, jpeg_io, out_info);
    GFX_GOTO_ON_FALSE(jpeg_ret == JPEG_ERR_OK, GFX_FAIL, err, TAG, "JPEG RGB888 header parse failed");
    ...
    jpeg_io->outbuf = out_data;
    jpeg_ret = jpeg_dec_process(jpeg_dec, jpeg_io);
    GFX_GOTO_ON_FALSE(jpeg_ret == JPEG_ERR_OK, GFX_FAIL, err, TAG, "JPEG RGB888 decode failed: %d", jpeg_ret);
        ...
        uint32_t width, uint32_t height, uint8_t *out_data, size_t *out_size)
{
    ...
    if (s_jpeg.hw_ready) {
        ...
        if (hw_ret != GFX_ERR_NOT_SUPPORTED) {
