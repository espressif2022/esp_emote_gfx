...
static void s_abs_deadline(uint32_t timeout_ms, struct timespec *out_ts)
{
...
static bool s_event_condition_met(gfx_platform_event_bits_t current,
...
{
    linux_event_t *event = calloc(1, sizeof(*event));
    if (event == NULL) {
        return NULL;
    }
...
{
    linux_event_t *event = (linux_event_t *)event_handle;
    if (event == NULL) {
        return;
    }
        ...
        bool clear_on_exit, bool wait_all, uint32_t timeout_ms)
{
    ...
    if (!s_event_condition_met(event->bits, bits, wait_all)) {
        if (timeout_ms == 0) {
            pthread_mutex_unlock(&event->mutex);
            return 0;
        }
        ...
        if (timeout_ms == GFX_PLATFORM_WAIT_FOREVER) {
            while (!s_event_condition_met(event->bits, bits, wait_all)) {
                pthread_cond_wait(&event->cond, &event->mutex);
            }
        } else {
            struct timespec deadline;
            s_abs_deadline(timeout_ms, &deadline);
            while (!s_event_condition_met(event->bits, bits, wait_all)) {
...
{
    linux_mutex_t *mutex = calloc(1, sizeof(*mutex));
    if (mutex == NULL) {
        return NULL;
    }
...
{
    linux_mutex_t *mutex = (linux_mutex_t *)mutex_handle;
    if (mutex == NULL) {
        return;
    }
...
static void *s_task_entry(void *arg)
{
    linux_task_start_t *start = (linux_task_start_t *)arg;
                                   ...
                                   void *arg, gfx_platform_task_t *out_task)
{
    ...
    linux_task_start_t *start = calloc(1, sizeof(*start));
    if (start == NULL) {
    ...
    pthread_t thread;
    int ret = pthread_create(&thread, &attr, s_task_entry, start);
