...
{
    return type == GFX_GSP_OBJ_CONTAINER || type == GFX_GSP_OBJ_LAYER ||
           type == GFX_GSP_OBJ_LABEL || type == GFX_GSP_OBJ_BUTTON ||
           type == GFX_GSP_OBJ_IMAGE || type == GFX_GSP_OBJ_LIST ||
           type == GFX_GSP_OBJ_WHEEL;
}

static uint16_t *decode_blob_rgb565(const uint8_t *gsp, size_t gfx_gsp_size,
                                    ...
                                    scratch_t *scratch)
{
    ...
    const uint8_t *b = gsp + blob_off + (size_t)blob_idx * GFX_GSP_BLOB_SIZE;
    ...
    if ((uint64_t)data_off + comp_size > gfx_gsp_size || raw_size == 0) {
        return NULL;
    }
                 ...
                 gfx_gsp_to_arena_info_t *info)
{
    if (gsp == NULL || out_pkg == NULL || out_pkg_size == NULL ||
            gfx_gsp_size < GFX_GSP_HEADER_SIZE) {
        return GFX_GSP_TO_ARENA_ERR_ARG;
    }

    const uint32_t magic = gfx_gsp_rd_u32(gsp + 0);
    const uint32_t version = gfx_gsp_rd_u32(gsp + 4);
    if (magic != GFX_GSP_MAGIC) {
        return GFX_GSP_TO_ARENA_ERR_MAGIC;
    }
    if (version != GFX_GSP_VERSION) {
        return GFX_GSP_TO_ARENA_ERR_VERSION;
    }

    const uint16_t screen_w = gfx_gsp_rd_u16(gsp + 8);
    const uint16_t screen_h = gfx_gsp_rd_u16(gsp + 10);
    const uint32_t screen_bg = gfx_gsp_rd_u32(gsp + 12);
    const uint32_t obj_count = gfx_gsp_rd_u32(gsp + 16);
    const uint32_t obj_off = gfx_gsp_rd_u32(gsp + 20);
    const uint32_t blob_count = gfx_gsp_rd_u32(gsp + 28);
    const uint32_t blob_off = gfx_gsp_rd_u32(gsp + 32);
    const uint32_t total_size = gfx_gsp_rd_u32(gsp + 36);

    if (total_size != gfx_gsp_size || obj_count == 0 || obj_count > 4096u) {
        return GFX_GSP_TO_ARENA_ERR_SIZE;
    }
    if ((uint64_t)obj_off + (uint64_t)obj_count * GFX_GSP_OBJ_SIZE > gfx_gsp_size) {
        return GFX_GSP_TO_ARENA_ERR_SIZE;
    }
    if (blob_count > 0 &&
            (uint64_t)blob_off + (uint64_t)blob_count * GFX_GSP_BLOB_SIZE > gfx_gsp_size) {
        return GFX_GSP_TO_ARENA_ERR_SIZE;
    }

    uint16_t *map = (uint16_t *)calloc(obj_count, sizeof(uint16_t));
    if (map == NULL) {
        return GFX_GSP_TO_ARENA_ERR_ALLOC;
    }
    ...
    for (uint32_t i = 0; i < obj_count; i++) {
        const uint8_t *e = gsp + obj_off + (size_t)i * GFX_GSP_OBJ_SIZE;
        const uint16_t type = gfx_gsp_rd_u16(e + 0);
        if (is_convertible(type)) {
            map[i] = out_n++;
            if (type == GFX_GSP_OBJ_IMAGE) {
                const uint32_t blob_idx = gfx_gsp_rd_u32(e + 44);
                if (blob_idx < blob_count) {
                    const uint8_t *b = gsp + blob_off + (size_t)blob_idx * GFX_GSP_BLOB_SIZE;
                    scratch_need += (size_t)gfx_gsp_rd_u32(b + 8) + 64u;
                }
            } else if (type == GFX_GSP_OBJ_LIST || type == GFX_GSP_OBJ_WHEEL) {
                const uint16_t plen = gfx_gsp_rd_u16(e + 52);
    ...
    if (out_n == 0) {
        free(map);
        return GFX_GSP_TO_ARENA_ERR_EMPTY;
    }

    scratch_t scratch = {0};
    scratch.size = scratch_need;
    scratch.mem = (uint8_t *)malloc(scratch.size);
    gfx_arena_desc_t *descs = (gfx_arena_desc_t *)calloc(out_n, sizeof(gfx_arena_desc_t));
    ...
    for (uint32_t i = 0; i < obj_count; i++) {
        ...
        const uint8_t *e = gsp + obj_off + (size_t)i * GFX_GSP_OBJ_SIZE;
        const uint16_t type = gfx_gsp_rd_u16(e + 0);
        const uint16_t parent_idx = gfx_gsp_rd_u16(e + 2);
        ...
        const char *text = (flags & GFX_GSP_F_TEXT) ? safe_str(gsp, gfx_gsp_size, text_off) : NULL;
        const char *cbname = (flags & GFX_GSP_F_CALLBACK) ? safe_str(gsp, gfx_gsp_size, cb_off) : NULL;
        const char *wname = (flags & GFX_GSP_F_NAME) ? safe_str(gsp, gfx_gsp_size, name_off) : NULL;
        const uint8_t *params = NULL;
        if ((flags & GFX_GSP_F_PARAMS) && params_off != 0 &&
                (size_t)params_off + params_len <= gfx_gsp_size) {
            params = gsp + params_off;
        }

        gfx_arena_desc_t *d = &descs[oi];
        memset(d, 0, sizeof(*d));
        d->x = x;
        d->y = y;
        d->w = w;
        d->h = h;
        d->flags = GFX_ARENA_F_VISIBLE;
        if (flags & GFX_GSP_F_HIDDEN) {
            d->flags = (uint16_t)(d->flags & ~GFX_ARENA_F_VISIBLE);
        }

        if (parent_idx == GFX_GSP_NO_PARENT) {
            d->parent = -1;
        ...
        if (type == GFX_GSP_OBJ_CONTAINER || type == GFX_GSP_OBJ_LAYER) {
            d->type = GFX_ARENA_NODE_CONTAINER;
            d->name = wname;
            if (flags & GFX_GSP_F_BG_COLOR) {
                d->flags |= GFX_ARENA_F_BG;
                d->bg_rgb = bg;
            }
        } else if (type == GFX_GSP_OBJ_LABEL) {
            d->type = GFX_ARENA_NODE_LABEL;
            d->name = (text != NULL) ? text : wname;
            d->bg_rgb = (flags & GFX_GSP_F_FG_COLOR) ? fg : 0xF3F7FAu;
        } else if (type == GFX_GSP_OBJ_BUTTON) {
            d->type = GFX_ARENA_NODE_BUTTON;
            d->name = (text != NULL) ? text : wname;
            d->action = cbname;
            d->flags |= GFX_ARENA_F_CLICKABLE;
            if (flags & GFX_GSP_F_BG_COLOR) {
                d->flags |= GFX_ARENA_F_BG;
                d->bg_rgb = bg;
            } else {
        ...
        } else if (type == GFX_GSP_OBJ_IMAGE) {
            d->type = GFX_ARENA_NODE_IMAGE;
            d->name = wname;
            if (flags & GFX_GSP_F_IMAGE) {
                uint16_t iw = 0, ih = 0;
                uint16_t *px = decode_blob_rgb565(gsp, gfx_gsp_size, blob_off, blob_count,
            ...
            d->type = (type == GFX_GSP_OBJ_LIST) ? GFX_ARENA_NODE_LIST : GFX_ARENA_NODE_WHEEL;
            d->name = wname;
            d->flags |= GFX_ARENA_F_CLICKABLE | GFX_ARENA_F_BG;
            d->bg_rgb = (flags & GFX_GSP_F_BG_COLOR) ? bg : 0x1a1f2eu;
            d->u.items.text_rgb = (flags & GFX_GSP_F_FG_COLOR) ? fg : 0xF3F7FAu;
            if (params != NULL) {
                const char **items = NULL;
                uint16_t ic = 0, sel = 0, ih = 0, iflags = 0;
                if (parse_items(params, params_len, &scratch, &items, &ic, &sel, &ih, &iflags) == 0) {
                    d->u.items.items = items;
                    d->u.items.item_count = ic;
                    d->u.items.selected = sel;
                    d->u.items.item_height = ih;
                    d->u.items.flags = 0;
                    if (type == GFX_GSP_OBJ_WHEEL && (iflags & GFX_GSP_ITEM_PARAMS_F_CYCLIC)) {
                        d->u.items.flags |= GFX_ARENA_ITEMS_F_CYCLIC;
                    }
                }
            }
        }
    ...
    size_t pkg_size = 0;
    uint8_t *pkg = gfx_arena_pack(descs, out_n, &pkg_size);
    free(descs);
    free(map);
    free(scratch.mem);

    if (pkg == NULL) {
        return GFX_GSP_TO_ARENA_ERR_PACK;
    }
