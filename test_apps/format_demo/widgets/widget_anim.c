...
static const demo_anim_clip_t s_anim_clips[] = {
    ...
    { .name = "SPIFFS AAF",      .path = DEMO_SPIFFS_ANIM_MOUNT "/mi_1_eye_24bit.aaf", .mode = "SPIFFS AAF" },
    { .name = "SPIFFS EAF",      .path = DEMO_SPIFFS_ANIM_MOUNT "/mi_1_eye_8bit.eaf", .mode = "SPIFFS EAF" },
};
...
static void demo_update_anim_note(size_t index)
{
    ...
    (void)snprintf(s_anim_note, sizeof(s_anim_note), "%s | %s | %s",
...
static void demo_mount_spiffs_assets(void)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGW(TAG_ANIM, "spiffs mount failed: %s; SPIFFS clips will be skipped",
                 esp_err_to_name(err));
    }
}

static esp_err_t demo_load_anim_clip(format_playground_scene_t *scene, size_t index)
{
    const demo_anim_clip_t *clip;
    gfx_anim_src_t src = {0};

    ESP_RETURN_ON_FALSE(scene != NULL && scene->anim_demo != NULL, ESP_ERR_INVALID_STATE,
                        "playground", "anim object is not ready");
    ESP_RETURN_ON_FALSE(index < DEMO_ANIM_COUNT, ESP_ERR_INVALID_ARG,
    ...
    ESP_RETURN_ON_ERROR(gfx_anim_set_src_desc(scene->anim_demo, &src),
                        "playground", "set anim src failed");
    ESP_RETURN_ON_ERROR(gfx_anim_set_segment(scene->anim_demo, 0, 0xFFFFFFFFU, 15, true),
                        "playground", "set anim segment failed");
    ESP_RETURN_ON_ERROR(gfx_anim_start(scene->anim_demo), "playground", "start anim failed");
...
esp_err_t gfx_format_demo_next_anim_clip(format_playground_scene_t *scene)
{
    return demo_load_anim_clip(scene, (s_anim_index + 1U) % DEMO_ANIM_COUNT);
}

esp_err_t gfx_format_demo_build_anim_demo(format_playground_scene_t *scene)
{
    gfx_object_t *obj;
    ...
    ESP_RETURN_ON_ERROR(demo_load_anim_clip(scene, 0), "playground", "load anim clip failed");
