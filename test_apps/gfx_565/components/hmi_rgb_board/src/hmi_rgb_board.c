...
static const uint8_t s_gt1151_addr_candidates[] = {0x5DU, 0x14U};
...
static esp_err_t hmi_rgb_board_touch_try_init_addr(uint8_t addr, esp_lcd_touch_handle_t *out_touch)
{
    esp_lcd_touch_config_t touch_config = {
        ...
        .rst_gpio_num = HMI_RGB_BOARD_TOUCH_RST,
        .int_gpio_num = HMI_RGB_BOARD_TOUCH_INT,
        .levels = {
            .reset = 0,
            .interrupt = 0,
        },
    ...
    ESP_RETURN_ON_ERROR(hmi_rgb_board_touch_select_addr(addr), TAG,
                        "select GT1151 address 0x%02x failed", addr);
...
esp_err_t hmi_rgb_board_touch_new(esp_lcd_touch_handle_t *out_touch)
{
    ...
    for (size_t i = 0; i < sizeof(s_gt1151_addr_candidates) / sizeof(s_gt1151_addr_candidates[0]); i++) {
        uint8_t addr = s_gt1151_addr_candidates[i];
        (void)hmi_rgb_board_touch_select_addr(addr);
        (void)hmi_rgb_board_touch_probe_product_id(addr);
        last_err = hmi_rgb_board_touch_try_init_addr(addr, out_touch);
        if (last_err == ESP_OK) {
            ESP_LOGI(TAG, "GT1151 touch initialized at I2C address 0x%02x", addr);
            return ESP_OK;
        }
        ESP_LOGW(TAG, "GT1151 probe at 0x%02x failed: %s", addr, esp_err_to_name(last_err));
    }
