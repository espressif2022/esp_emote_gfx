...
static uint32_t node_off_at_index(const gfx_arena_t *a, uint16_t idx)
{
    const gfx_arena_hdr_t *hdr = (const gfx_arena_hdr_t *)a->base;
    return hdr->nodes_off + (uint32_t)idx * (uint32_t)sizeof(gfx_arena_node_t);
}

static int parent_index_of(gfx_arena_t *a, uint16_t child_idx)
{
    ...
    for (uint16_t i = 0; i < hdr->node_count; i++) {
        gfx_arena_node_t *n = gfx_arena_node(a, node_off_at_index(a, i));
        for (uint32_t c = n->first_child; c != GFX_ARENA_NO_NODE; ) {
            if (c == child_off) {
                return (int)i;
            }
            gfx_arena_node_t *cn = gfx_arena_node(a, c);
            c = cn->next_sibling;
        }
    }
    return -1;
...
int gfx_arena_gfx_sync_node(gfx_arena_gfx_scene_t *scene, uint32_t node_off)
{
    ...
    const uint32_t idx = gfx_arena_node_index(&scene->arena, gfx_arena_node(&scene->arena, node_off));
    if (idx == GFX_ARENA_NO_NODE || idx >= scene->obj_count) {
        return -2;
    }

    gfx_arena_node_t *n = gfx_arena_node(&scene->arena, node_off);
    ...
    (void)gfx_object_set_visible(o, (n->flags & GFX_ARENA_F_VISIBLE) != 0);

    const char *name = gfx_arena_str(&scene->arena, n->name_off);

    switch (n->type) {
    case GFX_ARENA_NODE_CONTAINER:
        if (n->flags & GFX_ARENA_F_BG) {
            (void)gfx_container_set_bg_enable(o, true);
            (void)gfx_container_set_bg_color(o, GFX_COLOR_HEX(n->bg_rgb));
        }
        break;
    case GFX_ARENA_NODE_LABEL:
        if (scene->font != NULL) {
...
int gfx_arena_gfx_sync_all(gfx_arena_gfx_scene_t *scene)
{
    ...
    for (uint16_t i = 0; i < hdr->node_count; i++) {
        int rc = gfx_arena_gfx_sync_node(scene, node_off_at_index(&scene->arena, i));
        if (rc != 0) {
            return rc;
        }
    }
                   ...
                   gfx_font_t font, gfx_arena_gfx_scene_t *out)
{
    ...
    if (gfx_arena_load(pkg, pkg_size, &out->arena) != 0) {
        return -2;
    }

    const gfx_arena_hdr_t *hdr = gfx_arena_hdr(&out->arena);
    out->objs = (gfx_object_t **)calloc(hdr->node_count, sizeof(*out->objs));
    if (out->objs == NULL) {
        gfx_arena_free(&out->arena);
        return -3;
    }
    ...
    for (uint16_t i = 0; i < hdr->node_count; i++) {
        gfx_arena_node_t *n = gfx_arena_node(&out->arena, node_off_at_index(&out->arena, i));
        gfx_object_t *o = NULL;
        switch (n->type) {
        case GFX_ARENA_NODE_CONTAINER:
    ...
    if (gfx_arena_gfx_sync_all(out) != 0) {
        gfx_arena_gfx_free(out);
        return -7;
    }
    return 0;
...
gfx_object_t *gfx_arena_gfx_find_obj(gfx_arena_gfx_scene_t *scene, const char *name)
{
    ...
    gfx_arena_node_t *n = gfx_arena_find_by_name(&scene->arena, name);
    if (n == NULL) {
        return NULL;
    }
    const uint32_t idx = gfx_arena_node_index(&scene->arena, n);
    if (idx == GFX_ARENA_NO_NODE || idx >= scene->obj_count) {
        return NULL;
    }
...
void gfx_arena_gfx_free(gfx_arena_gfx_scene_t *scene)
{
    ...
    if (scene->objs != NULL) {
        ...
        const gfx_arena_hdr_t *hdr = scene->arena.base != NULL ? gfx_arena_hdr(&scene->arena) : NULL;
        if (hdr != NULL) {
