 ...
 *
 * 流程：
 *   1) #include GFX_GSP_SCENE_INC：导出的 GSP1 v4 二进制包（含动作表）；
 *   2) gfx_gsp_load() 只读包字节，按 type 工厂建树 + 解析动作表；
 *   3) 断言树结构/父子/坐标/动作表正确（自检）；
 *   4) SDL 渲染真实 esp_emote_gfx 画面。
 *
 * 交互（点击 OK 按钮）：
 *   - on_ok C 回调（GFX_GSP_F_CALLBACK）：计数并改标题文字；
 *   - 动作表（v4，纯数据，无需 C 代码）：给面板换底色 + 改副标题文字。
 *   二者由 loader 的统一 trampoline 依次分发，在同一控件上共存。
 *
 * 设 GFX_GSP_HEADLESS=1 则只做 1~3 步并按自检结果返回（可无窗口自动验证）。
 */
...
#define GFX_GSP_DEMO_SCREEN_W       HOME_CONTROL_SCREEN_W
#define GFX_GSP_DEMO_SCREEN_H       HOME_CONTROL_SCREEN_H
#define GFX_GSP_DEMO_OBJ_COUNT      HOME_CONTROL_OBJ_COUNT
#define GFX_GSP_DEMO_FONT_COUNT     HOME_CONTROL_FONT_COUNT
#define GFX_GSP_DEMO_ACTION_COUNT   HOME_CONTROL_ACTION_COUNT
#define GFX_GSP_DEMO_FONTS          home_control_fonts
#define GFX_GSP_DEMO_SCENE_PKG      home_control_scene_pkg
#define GFX_GSP_DEMO_SCENE_PKG_LEN  home_control_scene_pkg_len
#define GFX_GSP_DEMO_TITLE          "AI Scene (" GFX_GSP_SCENE_INC ")"
#elif defined(HOME_SCREEN_W)
#define GFX_GSP_DEMO_SCREEN_W       HOME_SCREEN_W
#define GFX_GSP_DEMO_SCREEN_H       HOME_SCREEN_H
#define GFX_GSP_DEMO_OBJ_COUNT      HOME_OBJ_COUNT
#define GFX_GSP_DEMO_FONT_COUNT     HOME_FONT_COUNT
#define GFX_GSP_DEMO_ACTION_COUNT   HOME_ACTION_COUNT
#define GFX_GSP_DEMO_FONTS          home_fonts
#define GFX_GSP_DEMO_SCENE_PKG      home_scene_pkg
#define GFX_GSP_DEMO_SCENE_PKG_LEN  home_scene_pkg_len
#define GFX_GSP_DEMO_TITLE          "AI Scene (" GFX_GSP_SCENE_INC ")"
#else
#error "Unsupported GSP scene include: missing HOME_* or HOME_CONTROL_* symbols"
#endif

#define SCREEN_W GFX_GSP_DEMO_SCREEN_W
#define SCREEN_H GFX_GSP_DEMO_SCREEN_H
...
static int create_demo_font(const gfx_gsp_font_desc_t *desc, demo_font_slot_t *slot)
{
...
static int create_demo_fonts(const gfx_gsp_font_desc_t *font_descs, uint16_t font_desc_count,
                             demo_font_slot_t **out_slots,
                             size_t *out_count, gfx_gsp_font_binding_t **out_bindings)
{
    ...
    gfx_gsp_font_binding_t *bindings = (gfx_gsp_font_binding_t *)calloc(font_desc_count, sizeof(*bindings));
...
{
    gfx_gsp_scene_t *scene = (gfx_gsp_scene_t *)user_data;

    (void)obj;
    if (event != NULL && event->type == GFX_TOUCH_EVENT_RELEASE) {
        s_clicks++;
        gfx_object_t *title = gfx_gsp_scene_find_by_name(scene, "title");
        if (title != NULL) {
...
static int verify_tree(const gfx_gsp_scene_t *scene)
{
    int fails = 0;
#define CHECK(cond, msg) do { if (!(cond)) { fprintf(stderr, "  FAIL: %s\n", msg); fails++; } } while (0)

    CHECK(scene->obj_count == GFX_GSP_DEMO_OBJ_COUNT, "obj_count == package obj count");
    CHECK(scene->root != NULL, "root != NULL");
    CHECK(scene->action_count == GFX_GSP_DEMO_ACTION_COUNT, "action_count == package action count");
    for (uint16_t i = 0; i < scene->obj_count; i++) {
        CHECK(scene->objs[i] != NULL, "object handle non-null");
    }
    if (scene->obj_count == GFX_GSP_DEMO_OBJ_COUNT) {
    ...
    if (scene->obj_count == 12 && gfx_gsp_scene_find_by_name(scene, "homeLayer") != NULL) {
        CHECK(gfx_gsp_scene_find_by_name(scene, "title") == gfx_gsp_scene_get_obj(scene, 2),
              "name lookup: title -> obj2");
        CHECK(gfx_gsp_scene_find_by_name(scene, "subtitle") == gfx_gsp_scene_get_obj(scene, 3),
              "name lookup: subtitle -> obj3");
        CHECK(gfx_gsp_scene_find_by_name(scene, "homeLayer") == gfx_gsp_scene_get_obj(scene, 1),
              "name lookup: homeLayer -> obj1");
        CHECK(gfx_gsp_scene_find_by_name(scene, "homeNext") == gfx_gsp_scene_get_obj(scene, 6),
              "name lookup: homeNext -> obj6");
        CHECK(gfx_gsp_scene_find_by_name(scene, "selectorLayer") == gfx_gsp_scene_get_obj(scene, 7),
              "name lookup: selectorLayer -> obj7");
        CHECK(gfx_gsp_scene_find_by_name(scene, "featureList") == gfx_gsp_scene_get_obj(scene, 9),
              "name lookup: featureList -> obj9");
        CHECK(gfx_gsp_scene_find_by_name(scene, "formatWheel") == gfx_gsp_scene_get_obj(scene, 10),
              "name lookup: formatWheel -> obj10");
        CHECK(gfx_gsp_scene_find_by_name(scene, "selectorNext") == gfx_gsp_scene_get_obj(scene, 11),
              "name lookup: selectorNext -> obj11");
        CHECK(gfx_gsp_scene_find_by_bind_id(scene, 1) == gfx_gsp_scene_get_obj(scene, 4),
              "bind lookup: bind_id 1 -> obj4");
        CHECK(gfx_object_get_visible(gfx_gsp_scene_get_obj(scene, 1)), "homeLayer initially visible");
        CHECK(!gfx_object_get_visible(gfx_gsp_scene_get_obj(scene, 7)), "selectorLayer initially hidden");
        CHECK(gfx_list_get_item_count(gfx_gsp_scene_get_obj(scene, 9)) == 4,
              "featureList has 4 params-v1 items");
        CHECK(gfx_wheel_get_item_count(gfx_gsp_scene_get_obj(scene, 10)) == 4,
              "formatWheel has 4 params-v1 items");
        CHECK(strcmp(gfx_wheel_get_item_text(gfx_gsp_scene_get_obj(scene, 10), 0), "RGB565") == 0,
...
int main(void)
{
    ...
    gfx_gsp_font_binding_t *font_bindings = NULL;
    size_t font_count = 0;

    if (create_demo_fonts(GFX_GSP_DEMO_FONTS, GFX_GSP_DEMO_FONT_COUNT,
                          &font_slots, &font_count, &font_bindings) != 0) {
        return 1;
    }

    /* 1) dump: the selected .inc already contains the compiled u32-offset package. */
    gfx_gsp_dump(GFX_GSP_DEMO_SCENE_PKG, GFX_GSP_DEMO_SCENE_PKG_LEN);
    ...
    gfx_gsp_scene_t scene = {0};
    const gfx_gsp_cb_binding_t cbs[] = {
        { .name = "on_ok", .cb = on_ok_cb, .user_data = &scene },
    };
    int rc = gfx_gsp_load_with_fonts(GFX_GSP_DEMO_SCENE_PKG, GFX_GSP_DEMO_SCENE_PKG_LEN, port.disp,
    ...
    printf(fails == 0 ? "SELF-CHECK: PASS (%u objects from %s)\n"
           ...
           scene.obj_count, GFX_GSP_SCENE_INC);

    if (headless) {
        (void)gfx_core_refresh_now(port.gfx);
        gfx_object_t *home_next = gfx_gsp_scene_find_by_name(&scene, "homeNext");
        gfx_object_t *selector_next = gfx_gsp_scene_find_by_name(&scene, "selectorNext");
        gfx_object_t *home_layer = gfx_gsp_scene_find_by_name(&scene, "homeLayer");
        gfx_object_t *selector_layer = gfx_gsp_scene_find_by_name(&scene, "selectorLayer");
        gfx_object_t *feature_list = gfx_gsp_scene_find_by_name(&scene, "featureList");
        gfx_object_t *format_wheel = gfx_gsp_scene_find_by_name(&scene, "formatWheel");
